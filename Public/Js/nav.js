...
$(function () {
	...
	$('#create_group').click(function()
		{
			...
			//绝对高度等于 滚动的高度加上窗口的居中高度
			var groupHeight=$(document).scrollTop()+($(window).height()-$('#add-group').height())/2;

			var group_obj=$('#add-group').show().css(
				...
				//创建个对象 并把其附加到group_head节点上
				createBg('group-bg');
				drag(group_obj,group_obj.find('group_head'));	
		});
	/*
	 *	当用户点击取消时,则隐藏对应的div
	 *	
	 */$('.group-cencle').click(function()
	 ...
	 $('.add-group-sub').click(function()
	 	...
	 	{
	 		var group_name=$('#gp-name').val();
	 		var data={'name':group_name};

	 		if(!group_name==''){
	 			//以POST的方式进行数据传送
	 			//接受服务器传回的数据状态码
	 			$.post(addGroup,data,function(data){
   ...
   $('.add-follow-sub').click(function()
   		{
   			...
   			var data={'follow':follow,'gid':gid};
   				
   			$.post(addFollow,data,function(data)
   				{
   					if(data.status){
   						$('.add-fl[uid=' + follow + ']').removeClass('add-fl').html('√&nbsp;已关注');
   						/*$('.list-right' dt).css('color':'#888888','background':'#ADD');*/
   						$('#follow').hide();
   						$('#follow-bg').remove();
   					}else{
   						alert(data.msg);
